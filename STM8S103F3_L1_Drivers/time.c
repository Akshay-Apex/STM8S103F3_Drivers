...
/*@Important: Works only when the HSI or HSE Oscillator is fmaster Source */
void time_delay_us(uint16_t us) {
  uint16_t fmaster_freq_khz = clk_fmaster_freq_khz_get();
  
  if(fmaster_freq_khz == 0 || us == 0) {
    return;
  }

  uint8_t divisor = fmaster_freq_khz / 1000U;  
  uint8_t prescaler = 0;
