...
inline void gpio_multi_mode_fast_config(GPIO_PORT_REG *port, GPIO_MULTI_MODE mode, uint8_t pin) {
...
inline uint8_t gpio_input_fast_read(GPIO_PORT_REG *port, uint8_t pin) {
...
inline void gpio_output_fast_set(GPIO_PORT_REG *port, uint8_t pin) {
  port->ODR |= (1U << pin);
}

void gpio_output_clear(GPIO_PORT_REG *port, uint8_t pin);

inline void gpio_output_fast_clear(GPIO_PORT_REG *port, uint8_t pin) {
  port->ODR &= ~(1U << pin);
}
...
inline uint8_t gpio_output_fast_read(GPIO_PORT_REG *port, uint8_t pin) {
...
inline void gpio_output_fast_toggle(GPIO_PORT_REG *port, uint8_t pin) {
  port->ODR ^= (1U << pin);
}
